...
__global__ void sharpen(int *d_data,int * d_result, int *d_filter, int sizeX, int sizeY, int fwidth) {
      ...
      if(y < sizeY && x <sizeX ) {
           ...
           for (yf=fwidth-1;yf>=0;yf--) {
               for (xf=fwidth-1;xf>=0;xf--) {
                        int xt= max(0,min(x-xf+hf,sizeX-1));
                        int yt= max(0,min(y-yf+hf,sizeY-1));
                        int w = d_filter[yf*fwidth+xf];
                        b += w*d_data[yt*sizeX+xt]; 
               }
           }
           d_result[y*sizeX+x]=b;
        }
}
...
void GPU_Test(int data[], int result[], int sizeX, int sizeY) {
	...
	dim3 dimBlock (16,16);
	dim3 dimGrid((sizeX+15)/16,(sizeY+15)/16);
	sharpen<<<dimGrid, dimBlock>>>(d_data,d_result,d_filter,sizeX,sizeY,FILTER_WIDTH);
	// Start timer for kernel
	// Don't change this function
	auto startKernel = chrono::steady_clock::now();

	cout<<"before sharp" <<endl;
	// TODO: call the kernel function
	sharpen<<<sizeY, sizeX>>>(d_data, d_result, d_filter,sizeX, sizeY,FILTER_WIDTH*FILTER_WIDTH);
...
void CPU_Test(int data[], int result[], int sizeX, int sizeY) {
	...
	int x,y,xf,yf,hf=FILTER_WIDTH/2;
	memset(result,0,sizeof(int)*sizeX*sizeY);
	for (y=0;y<sizeY;y++) {
		for (x=0;x<sizeX;x++) {
        		for (yf=FILTER_WIDTH-1;yf>=0;yf--) {
				for (xf=FILTER_WIDTH-1;xf>=0;xf--) {
                                        if ((y-yf+hf)<sizeY && (x-xf+hf)<sizeX && (y-yf+hf)>=0 && (x-xf+hf)>=0) {
						result[y*sizeX+x]+=data[(y-yf+hf)*sizeX + x-xf+hf]*
